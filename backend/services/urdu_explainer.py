...
async def explain_urdu(clause: str, clause_type: str = "", risk_level: str = "") -> str:
    ...
    FIX 1: Changed from sync def to async def so it does not block FastAPI's
            event loop when called from an async endpoint.
    FIX 2: Wraps the blocking Gemini SDK call in run_in_executor so other
            requests can be handled while waiting for the API response.
    FIX 3: Called with asyncio.gather() in analyze.py so all 8 clause
            explanations run concurrently instead of sequentially.
    """
...
Provide ONLY the Urdu explanation, nothing else. Start directly with the explanation."""
    ...
    try:
        # FIX: run_in_executor prevents blocking the async event loop
